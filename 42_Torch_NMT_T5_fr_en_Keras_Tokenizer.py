...
times = str(datetime.timedelta(seconds=Model_end_time)).split(".")
...
def sentence_process(input_sentence):
    text = SRC_tokenizer.texts_to_sequences([input_sentence])
...
def predict(sentence):
    out = sentence_process(sentence)
    out_list = out.tolist()
    out_list[0].pop(0)
    output_indexes = out_list[0]
    out_text = TRG_tokenizer.sequences_to_texts([output_indexes])
    return out_text

for num_idx in (11, 21, 31, 41, 51):
    print("Input        :", raw_src[num_idx])
    print("Prediction   :", predict(raw_src[num_idx]))
    print("Ground Truth :", raw_trg[num_idx],"\n")
