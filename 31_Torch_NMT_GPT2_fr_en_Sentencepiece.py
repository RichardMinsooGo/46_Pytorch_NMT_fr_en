...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs):
        ...
        for layer in self.layers:
            # (bs, n_dec_seq, hid_dim), (bs, n_dec_seq, n_dec_seq)
...
class GPT(nn.Module):
    ...
    def forward(self, dec_inputs):
        # (bs, n_seq, hid_dim), [(bs, n_heads, n_dec_seq, n_dec_seq)]
        dec_outputs, dec_self_attn_probs = self.decoder(dec_inputs)
        # (bs, n_dec_seq, n_dec_vocab), [(bs, n_heads, n_dec_seq, n_dec_seq)]
...
class Language_Model_Head(nn.Module):
    def __init__(self):
        ...
        self.gpt = GPT()
        # lm
        self.projection_lm = nn.Linear(hid_dim, n_dec_vocab, bias=False)
        self.projection_lm.weight = self.gpt.decoder.embedding.weight
    ...
    def forward(self, dec_inputs):
        ...
        # (bs, n_dec_seq - 1, n_dec_vocab), (bs, n_output), [(bs, n_heads, n_dec_seq, n_dec_seq)
        # return logits_lm[:, :-1, :].contiguous(), dec_self_attn_probs
        return logits_lm.contiguous(), dec_self_attn_probs
    ...
    def __init__(self, vocab_src, vocab_trg, infile):
        ...
        with open(infile, "r") as f:
            for i, line in enumerate(tqdm(f, total=line_cnt, desc=f"Loading {infile}", unit=" lines")):
                ...
                src_sentence = [self.vocab_src.piece_to_id("[BOS]")] + [self.vocab_src.piece_to_id(p) for p in data["SRC"]] + [self.vocab_trg.piece_to_id("[EOS]")]
                trg_sentence = []
...
def L_M_collate(inputs):
    src_inputs, trg_outputs = list(zip(*inputs))
...
def train_epoch(epoch, model, criterion, optimizer, train_loader):
    ...
    with tqdm_notebook(total=len(train_loader), desc=f"Train {epoch+1}") as pbar:
        for i, value in enumerate(train_loader):
            src_inputs, trg_outputs = map(lambda v: v.to(device), value)
            
            optimizer.zero_grad()
            outputs = model(src_inputs)
            logits_lm = outputs[0]
            
            # labels_lm_tmp = trg_outputs[:, 1:].contiguous()
